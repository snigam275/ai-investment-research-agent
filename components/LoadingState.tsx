...
const STEPS = [
  ...
  { 
    ...
    logs: [
      ...
      'Sending payloads to Google Gemini 1.5 Flash...',
...
export default function LoadingState({ company }: { company: string }) {
  ...
  useEffect(() => {
    // Progress steps every 3.5 seconds
  ...
  return (
    <div className="w-full max-w-2xl mx-auto space-y-8 py-6">
      ...
      <div className="grid grid-cols-5 gap-2 max-w-md mx-auto">
        {STEPS.map((_, i) => (
          <div key={i} className="h-1.5 rounded-full bg-slate-800 overflow-hidden">
            <div 
      ...
      {/* Terminal logs panel */}
      <div className="bg-slate-950/90 border border-slate-800/80 rounded-2xl shadow-2xl overflow-hidden glow-indigo">
        {/* Terminal Header */}
        <div className="flex items-center px-4 py-3 bg-slate-900/60 border-b border-slate-850 select-none">
        ...
        {/* Terminal Body */}
        <div className="p-5 font-mono text-[11px] text-slate-300 space-y-1.5 h-56 overflow-y-auto leading-relaxed select-text">
          {consoleLogs.map((log, index) => {
            let color = 'text-slate-400';
            if (log.includes('[system]')) color = 'text-indigo-400';
            else if (log.includes('[agent:searching]')) color = 'text-emerald-400';
            else if (log.includes('[agent:scanning]')) color = 'text-sky-400';
            else if (log.includes('[agent:analyzing]')) color = 'text-amber-400';
